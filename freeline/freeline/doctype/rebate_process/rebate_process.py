...
class RebateProcess(Document):
	...
	def validate(self):
		exist = frappe.db.sql(""" SELECT name FROM `tabRebate Process` where customer = %(customer)s and rebate_type = %(rebate_type)s 
									and name != %(name)s and sales_rep = %(sales_rep)s and rebate_duration = %(rebate_duration)s """,
		...
		for grp in self.rebate_details:
			...
			exist_brand = frappe.db.sql("""SELECT customer,d.brand 
											...
											where r.name = d.parent and r.customer = %(customer)s and d.brand = %(brand)s and r.name != %(name)s and r.sales_rep = %(sales_rep)s and rebate_duration = %(rebate_duration)s""",
