...
def get_hiuom(item_code):
	data = frappe.db.sql(
		"""
		SELECT uom FROM `tabUOM Conversion Detail` um where um.parent = %(item_code)s order by conversion_factor desc limit 1 """,{'item_code':item_code},as_dict=1)
...
def get_hiuom_factor(item_code):
	data = frappe.db.sql(
		"""
		SELECT conversion_factor FROM `tabUOM Conversion Detail` um where um.parent = %(item_code)s order by conversion_factor desc limit 1""",{'item_code':item_code},as_dict=1)
...
def format_report_data(filters: Filters, item_details: Dict, to_date: str) -> List[Dict]:
	...
	for item, item_dict in item_details.items():
		...
		if fifo_queue:
			...
			HiUOM = get_hiuom(details.name)
			HiUOM_Fact = get_hiuom_factor(details.name)
		...
		row.extend(
			[
				flt(item_dict.get("total_qty"), precision)/HiUOM_Fact,
				...
				range3/HiUOM_Fact,
				range4/HiUOM_Fact,
				above_range4/HiUOM_Fact,
				earliest_age,
				latest_age,
				last_rec_date,
				last_rec_qty/HiUOM_Fact,
				details.stock_uom,
				cost_and_sale[0].m1/HiUOM_Fact,
				cost_and_sale[0].m2/HiUOM_Fact,
				cost_and_sale[0].m3/HiUOM_Fact,
				cost_and_sale[0].m4/HiUOM_Fact,
				cost_and_sale[0].m5/HiUOM_Fact,
				cost_and_sale[0].m6/HiUOM_Fact,
				six_month_qty/HiUOM_Fact,
				usage_rate,
				stock_cover,
				turnover_rate,
				supplier_code
...
def get_columns(filters: Filters) -> List[Dict]:
	...
	columns.extend(
		[
			{"label": _("Qty on Hand (Hi-UOM)"), "fieldname": "qty", "fieldtype": "Float", "width": 170},
			...
			{"label": _("Hi-UOM"), "fieldname": "HiUOM", "fieldtype": "Data", "width": 120},
			{"label": _("Hi-UOM Factor"), "fieldname": "HiUOM_Fact", "fieldtype": "Float", "width": 130},
		]
	)
	...
	columns.extend(
		[
			...
			{"label": _("Latest Reciept Qty (Hi-UOM)"), "fieldname": "last_rec_qty", "fieldtype": "Float", "width": 170},
			{"label": _("Default UOM"), "fieldname": "uom", "fieldtype": "Link", "options": "UOM", "width": 140},
			{"label": _("Sale (0-30) Hi-UOM"), "fieldname": "m1", "fieldtype": "Float", "width": 140},
			{"label": _("Sale (31-60) Hi-UOM"), "fieldname": "m2", "fieldtype": "Float", "width": 140},
			{"label": _("Sale (61-90) Hi-UOM"), "fieldname": "m3", "fieldtype": "Float", "width": 140},
			{"label": _("Sale (91-120) Hi-UOM"), "fieldname": "m4", "fieldtype": "Float", "width": 140},
			{"label": _("Sale (121-150) Hi-UOM"), "fieldname": "m5", "fieldtype": "Float", "width": 140},
			{"label": _("Sale (151-180) Hi-UOM"), "fieldname": "m6", "fieldtype": "Float", "width": 140},
			{"label": _("Total Qty Sold in 6M Hi-UOM"), "fieldname": "six_month_qty", "fieldtype": "Float", "width": 160},
