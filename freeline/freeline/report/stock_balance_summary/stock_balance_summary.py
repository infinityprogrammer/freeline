...
def get_items(filters):
	...
	if filters.get("item_code"):
		...
		if filters.get("brand"):  # used in stock analytics report
			conditions.append("item.brand IN %(brand)s ")
	items = []
	if conditions:
...
def get_item_details(items, sle, filters):
	...
	if filters.get("brand"):
		brand_list = filters.get("brand")
		b_condition += " and item.brand in ('FAB')"
	print("-----b_condition--")
	print(b_condition)
	res = frappe.db.sql(
		"""
		...
		where
			item.name in (%s) %s
	"""
		% (cf_field, cf_join, ",".join(["%s"] * len(items)),b_condition),
		items,
		as_dict=1,
	)
