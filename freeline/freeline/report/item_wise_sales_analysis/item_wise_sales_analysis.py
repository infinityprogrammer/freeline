...
def _execute(filters=None, additional_table_columns=None, additional_query_columns=None):
	...
	for d in item_list:
		...
		row = {
			...
			"uom": d.uom,
			"qty": d.qty,
			"highest_uom": d.highest_uom,
			"highest_uom_factor": d.highest_uom_factor,
			"qty_highest_uom": round((d.stock_qty/d.highest_uom_factor),2),
			...
			"invoice_discount_amount": d.invoice_discount_amount,
...
def get_items(filters, additional_query_columns):
	...
	return frappe.db.sql(
		...
		select
			...
			round((`tabSales Invoice Item`.stock_qty / (SELECT conversion_factor FROM `tabUOM Conversion Detail` uf where uf.parent = `tabSales Invoice Item`.item_code and uf.uom = 'Carton')),2) as qty_in_carton,
			`tabSales Invoice Item`.`item_name`, `tabSales Invoice Item`.`item_group`,
			`tabSales Invoice Item`.sales_order, `tabSales Invoice Item`.delivery_note,
			`tabSales Invoice Item`.income_account, `tabSales Invoice Item`.cost_center,
			`tabSales Invoice Item`.stock_qty, `tabSales Invoice Item`.stock_uom,
			(SELECT uom FROM `tabUOM Conversion Detail` um where um.parent = `tabSales Invoice Item`.item_code order by conversion_factor desc limit 1)highest_uom,
			(SELECT conversion_factor FROM `tabUOM Conversion Detail` um where um.parent = `tabSales Invoice Item`.item_code order by conversion_factor desc limit 1)highest_uom_factor,
