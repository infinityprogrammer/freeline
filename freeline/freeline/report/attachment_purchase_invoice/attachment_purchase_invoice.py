...
def get_data(filters):
	...
	data = frappe.db.sql(
		"""
		SELECT company,p.name,posting_date,grand_total,
		(
		CASE
		...
		FROM `tabPurchase Invoice` p left join `tabFile` f on p.name = f.attached_to_name where 1=1 {0} order by posting_date desc""".format(conditions),filters,as_dict=1)
