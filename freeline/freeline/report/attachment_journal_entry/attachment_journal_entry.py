...
def get_data(filters):
	...
	data = frappe.db.sql(
		"""
		SELECT company,p.name,posting_date,total_debit as total,
		(CASE
			WHEN ifnull(file_url, '') = '' THEN '<span style="color:red">No Attachment</span>'
			ELSE concat("<a target ='_blank' href='",file_url,"'>Attachment</a>")
		END
		)file_url
		FROM `tabJournal Entry` p left join `tabFile` f on p.name = f.attached_to_name where 1=1 {0} order by posting_date desc""".format(conditions),filters,as_dict=1)
